...
def halves(conn, slot_id):
    rows = conn.execute(
        "SELECT a.handle, b.zone FROM escape_bookings b JOIN attendees a ON a.id=b.attendee_id "
        "WHERE b.slot_id=? AND b.status IN ('booked','checked_in')", (slot_id,)).fetchall()
    return (sum(1 for r in rows if r["zone"] == "A"),
            sum(1 for r in rows if r["zone"] == "B"))


# ---------------------------------------------------------------------------
# The halves stay playable (§9 r16, r22)
# ---------------------------------------------------------------------------

def test_halves_even_out_when_friends_are_removed(world):
    """A six that loses two from one half used to arrive 3 and 1.

    The game is built so that neither half finishes alone — each finds
    things the other can't see — so an uneven split is a broken game, not
    just an untidy one.
    """
    slot = first_game(world)
    owner = pid(world, "bananabelles")
    bookings.book(world, owner, slot["id"],
                  ["heidily", "t_shixuan", "bingkiat", "zhnlun", "joncjy"], NOW)
    assert halves(world, slot["id"]) == (3, 3)

    gone = [r["handle"] for r in world.execute(
        "SELECT a.handle FROM escape_bookings b JOIN attendees a ON a.id=b.attendee_id "
        "WHERE b.slot_id=? AND b.zone='B' AND b.status='booked'", (slot["id"],))
        if r["handle"] != "bananabelles"][:2]
    for h in gone:
        bookings.remove_friend(world, owner, h, NOW)

    a, b = halves(world, slot["id"])
    assert abs(a - b) <= 1, f"halves left at A={a} B={b}"
    assert a + b == 4


def test_both_halves_still_exist_after_a_removal(world):
    """Four booked, two leave: two must remain, one in each half, or one side
    of the room has nobody in it."""
    slot = first_game(world)
    owner = pid(world, "bananabelles")
    bookings.book(world, owner, slot["id"], ["heidily", "t_shixuan", "bingkiat"], NOW)
    for h in ("heidily", "t_shixuan"):
        bookings.remove_friend(world, owner, h, NOW)
    a, b = halves(world, slot["id"])
    assert (a, b) == (1, 1)


def test_halves_are_left_alone_once_the_game_has_started(world):
    """§9 r22 — after the start only the GM moves anyone, and every move is
    logged. An automatic tidy-up here would move a player mid-game."""
    slot = first_game(world)
    owner = pid(world, "bananabelles")
    bookings.book(world, owner, slot["id"], ["heidily", "t_shixuan", "bingkiat"], NOW)
    game.act(world, slot["id"], "start", GM, NOW)
    before = {r["id"]: r["zone"] for r in world.execute(
        "SELECT id, zone FROM escape_bookings WHERE slot_id=?", (slot["id"],))}
    bookings._assign_halves(world, slot["id"])
    after = {r["id"]: r["zone"] for r in world.execute(
        "SELECT id, zone FROM escape_bookings WHERE slot_id=?", (slot["id"],))}
    assert before == after


def zone_of(conn, slot_id, handle):
    return conn.execute(
        "SELECT b.zone FROM escape_bookings b JOIN attendees a ON a.id=b.attendee_id "
        "WHERE b.slot_id=? AND a.handle=? AND b.status IN ('booked','checked_in')",
        (slot_id, handle)).fetchone()["zone"]


# ---------------------------------------------------------------------------
# Friends stay together (organiser's instruction, 18 Sep — replaces §9 r16's
# "each new player into the smaller half", which split every group)
# ---------------------------------------------------------------------------

def test_two_groups_sharing_a_game_are_not_mixed(world):
    """The case the organiser objected to: booking with friends and being
    split up when there was no need for it."""
    slot = first_game(world)
    bookings.book(world, pid(world, "bananabelles"), slot["id"],
                  ["heidily", "t_shixuan"], NOW)
    bookings.book(world, pid(world, "zhnlun"), slot["id"], ["joncjy"], NOW)

    first = {zone_of(world, slot["id"], h) for h in ("bananabelles", "heidily", "t_shixuan")}
    second = {zone_of(world, slot["id"], h) for h in ("zhnlun", "joncjy")}
    assert len(first) == 1, "the first group was split"
    assert len(second) == 1, "the second group was split"
    assert first != second, "both groups landed in the same half"


def test_a_group_alone_in_a_game_is_still_split(world):
    """The one case where splitting is right: there is nobody else to fill the
    other half, and a game with an empty desk cannot be played at all."""
    slot = first_game(world)
    bookings.book(world, pid(world, "bananabelles"), slot["id"],
...
def test_the_bigger_group_is_the_one_kept_whole(world):
    """When they cannot both be whole, keep the group a split would hurt most."""
    slot = first_game(world)
    bookings.book(world, pid(world, "bananabelles"), slot["id"],
                  ["heidily", "t_shixuan", "bingkiat", "zhnlun"], NOW)   # five
    bookings.book(world, pid(world, "joncjy"), slot["id"], [], NOW)      # one
    big = {zone_of(world, slot["id"], h)
           for h in ("bananabelles", "heidily", "t_shixuan", "bingkiat", "zhnlun")}
    assert len(big) == 1
    assert zone_of(world, slot["id"], "joncjy") not in big


def test_a_full_group_of_six_takes_one_whole_half(world):
    """Halves are six each, and `max_party` is six, so the largest group the
    app allows fits in one half exactly — the case this change is really for."""
    slot = first_game(world)
    six = ["heidily", "t_shixuan", "bingkiat", "zhnlun", "joncjy"]
    bookings.book(world, pid(world, "bananabelles"), slot["id"], six, NOW)
    bookings.book(world, pid(world, "sharmaineangg"), slot["id"], ["mr_rishieparker"], NOW)
    big = {zone_of(world, slot["id"], h) for h in ["bananabelles"] + six}
    assert len(big) == 1, "a full group of six was split even though a half holds six"
    assert halves(world, slot["id"]) in ((6, 2), (2, 6))


def test_a_group_bigger_than_a_half_spills_only_the_overflow(world):
    """Reachable only if the seat count is lowered: halves of two, a group of
    three. Two stay together and one spills, rather than splitting evenly."""
    slot = first_game(world)
    bookings.book(world, pid(world, "bananabelles"), slot["id"],
                  ["heidily", "t_shixuan"], NOW)
    bookings.book(world, pid(world, "zhnlun"), slot["id"], [], NOW)
    world.execute("UPDATE slots SET capacity=4 WHERE id=?", (slot["id"],))
    bookings._assign_halves(world, slot["id"], force=True)
    mine = [zone_of(world, slot["id"], h) for h in ("bananabelles", "heidily", "t_shixuan")]
    assert sorted(mine) in (["A", "A", "B"], ["A", "B", "B"]), mine
    assert halves(world, slot["id"]) == (2, 2)


def test_adding_a_friend_later_keeps_the_group_together(world):
    slot = first_game(world)
    owner = pid(world, "bananabelles")
    bookings.book(world, owner, slot["id"], ["heidily"], NOW)
    bookings.book(world, pid(world, "zhnlun"), slot["id"], ["joncjy"], NOW)
    bookings.add_friends(world, owner, ["t_shixuan"], NOW)
    mine = {zone_of(world, slot["id"], h) for h in ("bananabelles", "heidily", "t_shixuan")}
    assert len(mine) == 1, "adding a friend split the group"


def test_a_removal_does_not_strand_the_rest_of_a_group(world):
    slot = first_game(world)
    owner = pid(world, "bananabelles")
    bookings.book(world, owner, slot["id"], ["heidily", "t_shixuan"], NOW)
    bookings.book(world, pid(world, "zhnlun"), slot["id"], ["joncjy"], NOW)
    bookings.remove_friend(world, owner, "heidily", NOW)
    mine = {zone_of(world, slot["id"], h) for h in ("bananabelles", "t_shixuan")}
    theirs = {zone_of(world, slot["id"], h) for h in ("zhnlun", "joncjy")}
    assert len(mine) == 1 and len(theirs) == 1 and mine != theirs


def test_the_desk_half_is_never_empty_when_two_or_more_are_booked(world):
    """Whatever the grouping does, somebody has to be able to reach the phone."""
    slot = first_game(world)
    for friends, owner in ((["heidily"], "bananabelles"),
                           ([], "zhnlun"),
                           (["joncjy"], "t_shixuan")):
        bookings.book(world, pid(world, owner), slot["id"], friends, NOW)
    a, b = halves(world, slot["id"])
    assert a > 0 and b > 0, f"one half is empty: A={a} B={b}"


def test_the_gm_rebalance_uses_the_same_rule(world):
    """Otherwise 'even out the halves' would quietly undo the grouping."""
    slot = first_game(world)
    bookings.book(world, pid(world, "bananabelles"), slot["id"],
                  ["heidily", "t_shixuan"], NOW)
    bookings.book(world, pid(world, "zhnlun"), slot["id"], ["joncjy"], NOW)
    game.act(world, slot["id"], "start", GM, NOW)
    game.halves(world, slot["id"], GM, rebalance=True)
    mine = {zone_of(world, slot["id"], h) for h in ("bananabelles", "heidily", "t_shixuan")}
    assert len(mine) == 1
